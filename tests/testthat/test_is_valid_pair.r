...
test_that("general test cases", {
  ...
  out <- is_valid_pair(.id=10, .id2=12, .time=10, .time2=50, risk_period=30,
                       .min_t=0, .min_t2=0, .max_t=1000, .max_t2=1000,
                       bounds="[]")
  expect_true(out)
  ...
  out <- is_valid_pair(.id=10, .id2=12, .time=10, .time2=50, risk_period=30,
                       .min_t=0, .min_t2=0, .max_t=60, .max_t2=1000,
                       bounds="[]")
  expect_true(out)

  # control period could not be observed at all due to right-censoring
  out <- is_valid_pair(.id=10, .id2=12, .time=10, .time2=50, risk_period=30,
                       .min_t=0, .min_t2=0, .max_t=40, .max_t2=1000,
                       bounds="[]")
  expect_false(out)
  ...
  out <- is_valid_pair(.id=10, .id2=12, .time=10, .time2=50, risk_period=30,
                       .min_t=0, .min_t2=0, .max_t=40, .max_t2=1000,
                       bounds="[]", check_censoring=FALSE)
  expect_true(out)

  # one control period cannot be observed at all, the risk period only partially
  out <- is_valid_pair(.id=10, .id2=12, .time=10, .time2=50, risk_period=30,
                       .min_t=0, .min_t2=0, .max_t=35, .max_t2=70, bounds="[]")
  expect_false(out)

  # two censoring times, both should be fine
  out <- is_valid_pair(.id=10, .id2=12, .time=10, .time2=50, risk_period=30,
                       .min_t=0, .min_t2=0, .max_t=75, .max_t2=70, bounds="[]")
  expect_true(out)

  # allow_overlap works
  out <- is_valid_pair(.id=2, .id2=3, .time=20, .time2=50, risk_period=40,
                       .min_t=0, .min_t2=0, .max_t=1000, .max_t2=1000,
                       bounds="(]", allow_overlap=TRUE)
  expect_true(out)

  # true if observation starts too late, but is not checked
  out <- is_valid_pair(.id=2, .id2=3, .time=20, .time2=100, risk_period=40,
                       .min_t=15, .min_t2=50, .max_t=1000, .max_t2=1000,
                       bounds="(]", check_truncation=FALSE)
  expect_true(out)

  # false if observation starts too late, and is checked
  out <- is_valid_pair(.id=2, .id2=3, .time=20, .time2=100, risk_period=40,
                       .min_t=15, .min_t2=50, .max_t=1000, .max_t2=1000,
                       bounds="(]", check_truncation=TRUE)
  expect_false(out)
})
...
test_that("works with NA", {
  ...
  out <- is_valid_pair(.id=10, .id2=20, .time=1000, .time2=NA, risk_period=30,
                       .min_t=0, .min_t2=0, .max_t=10000, .max_t2=10000,
                       bounds="[]")
  expect_true(is.na(out))
})
